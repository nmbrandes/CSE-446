...
def robust_rmtree(path):
    """
    Robustly removes a directory tree, handling read-only files.
    """
    def onerror_handler(func, path, exc_info):
        ...
        it attempts to change the file permission and retry.
        """
        if not os.access(path, os.W_OK):
            os.chmod(path, stat.S_IWUSR)  # Make it writable
            func(path)  # Retry the failed function (e.g., os.remove or os.rmdir)
        else:
    ...
    if os.path.exists(path):
        try:
            shutil.rmtree(path, onerror=onerror_handler)
        except Exception as e:
            # Print a warning if removal still fails after retries/permission changes
            print(f"Warning: Could not fully remove directory {path}: {e}")
