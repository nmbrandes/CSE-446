...
def vanilla_solution(x: Vector, y: Vector, A: Matrix, B: Matrix) -> Vector:
    ...
    """
    The gradient of f(x, y) with respect to x is
    $$nabla_x f(x, y) = A x + A^T x + B^T y$$
    """
    Ax = vanilla_matmul(A, x)
    ATx = vanilla_matmul(vanilla_transpose(A), x)
    BTy = vanilla_matmul(vanilla_transpose(B), y)
    grad = [Ax_i + ATx_i + BTy_i for Ax_i, ATx_i, BTy_i in zip(Ax, ATx, BTy)]
